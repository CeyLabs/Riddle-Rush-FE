...
interface AppState {
  ...
  viewMode: "grid" | "list";
  addCampaign: (
    campaign: Omit<Campaign, "id" | "createdAt" | "questionCount">,
  ) => string;
    ...
    (set, get) => ({
      ...
      questions: [
        ...
        },
        {
          ...
          campaignId: "4",
          question:
            "What does DeFi stand for and what is its main purpose in the blockchain ecosystem?",
          answerType: "ai-validated",
          answer:
      ...
      deleteCampaign: (id) => {
        set((state) => ({
          ...
          questions: state.questions.filter(
            (question) => question.campaignId !== id,
          ),
        }));
      },
      ...
      addQuestion: (questionData) => {
        ...
        const question: Question = {
          ...
          id,
          status:
            new Date(questionData.startTime) > new Date()
              ? "upcoming"
              : "active",
        };
        set((state) => ({
          ...
          set((state) => ({
            ...
            campaigns: state.campaigns.map((campaign) =>
              campaign.id === question.campaignId
                ? {
                    ...campaign,
                    questionCount: Math.max(0, campaign.questionCount - 1),
                  }
                : campaign,
            ),
          }));
      ...
      getCampaignQuestions: (campaignId) => {
        return get().questions.filter(
          (question) => question.campaignId === campaignId,
        );
      },
